...
var bi_formats;
var gabcStar = window.__mockLocalStorage.gabcStar || '<v>\\greheightstar</v>';
var gabcFlex = window.__mockLocalStorage.gabcFlex || sym_flex;
...
var o_g_tones={}; var g_tones = {'1.':{clef:"c4",
...
var Syl = (function(){
  ...
  return {
    words:(window.window.__mockLocalStorage&&window.window.__mockLocalStorage.words&&JSON.parse(window.window.__mockLocalStorage.words))||{},
    queue:[],
    syllabify:function(text,lang){
      ...
      while((m=regexWords.exec(text)) && m[0]){
        var raw=(m[5]?(m[4]+m[5]+m[6]):m[3]),
            w=raw.toLowerCase()
            var opi=m[4]?m[4].length:0,         // opening parenthesis index
    ...
    addResult:function(list){
      ...
      if(window.window.__mockLocalStorage)window.window.__mockLocalStorage.words=JSON.stringify(this.words);
    }
  }
})();
function syllable(match,index,bi) {
  ...
  if(typeof(match)=="string"){
    ...
    var oTags=(function(t){return (t||"").match(/<[^>]+>/g)||[];})(match[1]);
    var cTags=(function(t){return (t||"").match(/<[^>]+>/g)||[];})(match[9]);
    var tmp;
...
function applyPsalmTone(options) {
  var experimentalOxytones = typeof new URLSearchParams(window.location.search).get('experimental') === 'string';
  ...
  while(!finished){
    ...
    for(var ti = tones.length - 1; (ti >= 0 || lastOpen) && si >= 0; --ti,--si) {
      ...
      if(ti == tones.length - 1 && si == syl.length - 1 && toneList.accents > 0 && s.accent){
        ...
        if (ti === undefined) ti = tones.length - 1;
        if(tones[ti] && !tones[ti].open){
      ...
      if(tone && tone.open && !tone.accent) {
        ...
        if(italic && ti>2 && tones[ti-1] && tones[ti-1].open && tones[ti-2] && tones[ti-2].accent) {
          // if this is the end of a series of unaccented syllables following a prior accent, we don't want to mark it:
          italic = false;
        }
        if(italic) {
          if(onlyVowel) {
            if(ti>0 && tones[ti-1] && tones[ti-1].open && (vow = regexVowel.exec(s.syl))) {
    ...
    if(!result.shortened && ti>=0){
      if(ti==0 && tones[ti] && tones[ti].open) {
        finished = true;
      } else {
...
function getGabcTones(gabc,prefix,flexEqualsTenor,clef) {
  clef = clef || (typeof _clef !== 'undefined' ? _clef : '') || 'c4';
    ...
    var t = $.ajax({url:"psalms/" + psalmNum + ".txt",
      ...
      success: function(data) {
        if(data.responseText != undefined) {
          ...
          var temp = temp[pI].innerHTML.split('\n');
...
window.g_tones = g_tones;
window['getGabcTones'] = getGabcTones;
window['removeIntonation'] = removeIntonation;
